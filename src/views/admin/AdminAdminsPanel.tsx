      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-4 flex items-center gap-4">
            <AvatarPreview src={adminAvatarDraft} label={meta.currentAdmin?.name || 'Admin'} large />
            <div className="min-w-0 flex-1 space-y-3">
              <label className="block text-xs font-black text-slate-500">
                <span className="mb-1 block">后台头像链接</span>
                <input value={adminAvatarDraft} onChange={(event) => setAdminAvatarDraft(event.target.value)} placeholder="https://... 或 data:image/..." className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-900" />
              </label>
